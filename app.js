...
function updateClassCountdown() {
  const remaining = secondsUntilEnd(state.countdown?.endTime || "");
  countdownDisplay.textContent = remaining === null ? "--:--" : formatRemaining(remaining);
}
...
async function renderChecklist({listName, containerId, mediaKind, rowClass, picClass, textClass, removeClass, placeholder, markerTarget}) {
  ...
  for(let i=0;i<list.length;i++) {
    ...
    row.className = `${rowClass}${blank ? " is-blank" : ""}${hasImage ? "" : " no-image"}`;
    row.innerHTML = `
...
async function renderAllLists() {
  ...
  await renderChecklist({
    ...
    textClass:"step-text",removeClass:"step-remove",placeholder:"Type a success criterion...",markerTarget:"success"
  });
  await renderChecklist({
    listName:"reminders",containerId:"#reminderList",mediaKind:"reminder",
    rowClass:"reminder-row",picClass:"reminder-pic",
    textClass:"reminder-text",removeClass:"reminder-remove",placeholder:"Type a reminder...",markerTarget:"reminders"
  });
  updateListVisibility();
